    ...
    public function index(){
        ...
        if(isset($_POST['show'])) {
            ...
            foreach ($fields as $cost) {
                ...
                $query = $this->db->query("select cost_field from cost where date   $date_range and trash=0  group by cost_field order by cost_field asc");
                $cost=0;
                $length=1;
                foreach($query->result() as $rows){
                        $cost_field = $rows->cost_field;
                        $this->data['cost_field'][$length] = $cost_field;
                       for($m=0;$m<=11;$m++){
                            $month=$months[$m];
                            $sql ="select amount from cost where date $month and cost_field = '$cost_field' and trash=0";
