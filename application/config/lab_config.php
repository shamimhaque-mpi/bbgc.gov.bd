...
$config['description'] = array(
    'H.S.C(1st Year)Admission',
    'H.S.C(2nd Year)Session Charge',
    'H.S.C(2nd Year)Form Fill up(Regular)',
    'H.S.C(2nd Year)Form Fill up(Irregular)',
    'H.S.C-B.M(1st Year)Admission',
    'H.S.C-B.M(2nd Year)Session Charge',
    'H.S.C-B.M(1st Year)Form Fill up(Regular)',
    'H.S.C-B.M(1st Year)Form Fill up(Irregular)',
    'H.S.C-B.M(2nd Year)Form Fill up(Regular)',
    'H.S.C-B.M(2nd Year)Form Fill up(Irregular)',
    'Degree(Pass) 1st year Admission',
    'Degree(Pass) 1st year Form fill up Regular',
    'Degree(Pass) 1st year Form fill up Irregular',
    'Degree(Pass) 2nd year Session Charge',
    'Degree(Pass) 2nd year Form Fill up Regular',
    'Degree(Pass) 2nd year Form Fill up Irregular',
    'Degree(Pass) 3rd year Session Charge',
    'Degree(Pass) 3rd year Form Fill up Regular',
    'Degree(Pass) 3rd year Form Fill up Irregular'
);
...
$config['designation_staff'] = array(
    'office_assistant'                  => 'Office Assistant',
    'Office_Head_Accountant'            => 'Office Head Accountant',
    'Office_Accountant_Assistant_(C.T)' => 'Office Accountant Assistant (C.T)',
    'Lab_Assistant'                     => 'Lab Assistant',
    'Typing_Lab_Assistant'              => 'Typing Lab Assistant',
    'hostel_super'                      => 'Hostel Super',
    'cooker'                            => 'Cooker',
    'security_gard'                     => 'Security Gard',
    'Computer_Operator'                 => 'Computer Operator',
    '4th_Class_Employee'                => '4th Class Employee',
);
...
$config['classes'] = array(
    ...
    'HSC(Open) 1st' => 'HSC(Open) 1st',
    'HSC(Open) 2nd' => 'HSC(Open) 2nd',
...
$civics_total = 0;
$subject_civics_one     = count($CI->action->read('online_admission', ['compulsory_subject_four'=>'CIVICS & GOOD GOVERNANCE']));
$subject_civics_two     = count($CI->action->read('online_admission', ['compulsory_subject_five'=>'CIVICS & GOOD GOVERNANCE']));
$subject_civics_three   = count($CI->action->read('online_admission', ['compulsory_subject_six'=>'CIVICS & GOOD GOVERNANCE']));

$civics_total = $subject_civics_one + $subject_civics_two + $subject_civics_three;


$social_total = 0;
$subject_social_one         = count($CI->action->read('online_admission', ['compulsory_subject_four'=>'Social Work']));
$subject_social_two         = count($CI->action->read('online_admission', ['compulsory_subject_five'=>'Social Work']));
$subject_social_three       = count($CI->action->read('online_admission', ['compulsory_subject_six'=>'Social Work']));

$social_total =  $subject_social_one + $subject_social_two + $subject_social_three;

$history_total = 0;

$subject_history_one        = count($CI->action->read('online_admission', ['compulsory_subject_four'=>'History']));
$subject_history_two        = count($CI->action->read('online_admission', ['compulsory_subject_five'=>'History']));
$subject_history_three      = count($CI->action->read('online_admission', ['compulsory_subject_six'=>'History']));

$history_total = $subject_history_one + $subject_history_two + $subject_history_three;


$geography_total = 0;

$subject_geography_one      = count($CI->action->read('online_admission', ['compulsory_subject_four'=>'Geography']));
$subject_geography_two      = count($CI->action->read('online_admission', ['compulsory_subject_five'=>'Geography']));
$subject_geography_three      = count($CI->action->read('online_admission', ['compulsory_subject_six'=>'Geography']));

$geography_total = $subject_geography_one + $subject_geography_two + $subject_geography_three;

$economics_total = 0;
$subject_economics_one      = count($CI->action->read('online_admission', ['compulsory_subject_four'=>'ECONOMICS']));
$subject_economics_two      = count($CI->action->read('online_admission', ['compulsory_subject_five'=>'ECONOMICS']));
$subject_economics_three      = count($CI->action->read('online_admission', ['compulsory_subject_six'=>'ECONOMICS']));

$economics_total = $subject_economics_one + $subject_economics_two + $subject_economics_three;

$logic_total = 0;

$subject_logic_one          = count($CI->action->read('online_admission', ['compulsory_subject_four'=>'LOGIC']));
$subject_logic_two          = count($CI->action->read('online_admission', ['compulsory_subject_five'=>'LOGIC']));
$subject_logic_three          = count($CI->action->read('online_admission', ['compulsory_subject_six'=>'LOGIC']));

$logic_total = $subject_logic_one + $subject_logic_two +$subject_logic_three; 

$sociology_total = 0;
$subject_sociology_one      = count($CI->action->read('online_admission', ['compulsory_subject_four'=>'SOCIOLOGY']));
$subject_sociology_two      = count($CI->action->read('online_admission', ['compulsory_subject_five'=>'SOCIOLOGY']));
$subject_sociology_three      = count($CI->action->read('online_admission', ['compulsory_subject_six'=>'SOCIOLOGY']));

$sociology_total = $subject_sociology_one + $subject_sociology_two + $subject_sociology_three;

$islam_total = 0; 

$subject_islam_one          = count($CI->action->read('online_admission', ['compulsory_subject_four'=>'ISLAMIC HISTORY AND CULTURE']));
$subject_islam_two          = count($CI->action->read('online_admission', ['compulsory_subject_five'=>'ISLAMIC HISTORY AND CULTURE']));
$subject_islam_three          = count($CI->action->read('online_admission', ['compulsory_subject_six'=>'ISLAMIC HISTORY AND CULTURE']));

$islam_toatal = $subject_islam_one + $subject_islam_two + $subject_islam_three;

$subject = $config['subjects'];

if($civics_total >= 300){
    unset($subject['humanities']['compolsury'][0][0]);
}

if($social_total >= 300){
    unset($subject['humanities']['compolsury'][0][1]);
}

if($sociology_total >= 200){
    unset($subject['humanities']['compolsury'][0][6]);
}

if($geography_total >= 200){
    unset($subject['humanities']['compolsury'][0][3]);
}

if($economics_total >= 100){
    unset($subject['humanities']['compolsury'][0][4]);
}

if($history_total >= 100){
    unset($subject['humanities']['compolsury'][0][2]);
}

if($islam_total >= 125){
    unset($subject['humanities']['compolsury'][0][7]);
}

if($logic_total >= 102){
    unset($subject['humanities']['compolsury'][0][5]);
}

$agriculture                = count($CI->action->read('online_admission', ['optional_subject'=>'Agriculture Education', 'group'=>'humanities']));
$psychology                 = count($CI->action->read('online_admission', ['optional_subject'=>'Psychology', 'group'=>'humanities']));
$islamic_studies            = count($CI->action->read('online_admission', ['optional_subject'=>'Islamic Studies', 'group'=>'humanities']));

if($agriculture >= 200){
    unset($subject['humanities']['optional'][0]);
}
if($psychology >= 135){
    unset($subject['humanities']['optional'][1]);
}
if($islamic_studies >= 130){
    unset($subject['humanities']['optional'][2]);
}
...
//Subject Edit Don't delete this X

$config['edit_subjects'] = array(
	'compolsury' => array(
		array('BANGLA', 'বাংলা', '101'),
		array('ENGLISH', 'ইংরেজী', '107'),
		array('ICT', 'তথ্য ও যোগাযোগ প্রযুক্তি', '275'),
